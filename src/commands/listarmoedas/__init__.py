  ...
  def run(self, **kwargs):
    ...
    for coin in data:
      if coin['symbol'] in coins_in_json:
        json_coins['coins'][coins_in_json.index(coin['symbol'])] = {'symbol': coin['symbol'], 'last_price': coin['last_price']}
      else:
        json_coins['coins'].append({'symbol': coin['symbol'], 'last_price': coin['last_price']})
    ...
    for x in json_coins['coins']:
      text += '{:<{}} - {}\n'.format(x['symbol'], bigger_name_size, x['last_price'])
